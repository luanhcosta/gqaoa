...
from gqaoa.config import DEFAULT_PROBLEM_ID, ProblemConfig
...
def test_build_problem_with_default_problem_id_bootstraps_and_matches_f_return_cov():
    # BEST_KNOWN_CONFIG and every CLI default to problem_id=DEFAULT_PROBLEM_ID
    # rather than None — this is the path they actually exercise. Nothing is
    # persisted yet in the isolated PROBLEMS_DIR, so build_problem() must
    # bootstrap it on first use (see ensure_default_problem_persisted), and the
    # resulting numbers must still match f_return_cov() exactly.
    problem = ProblemConfig(q=0.3, B=5, lamb=0, sdp=False, problem_id=DEFAULT_PROBLEM_ID)
